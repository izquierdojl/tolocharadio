...
function SierraIllustration() {
  return (
    <svg
      viewBox="0 0 640 360"
      className="w-full max-w-2xl text-pine-200"
      role="img"
      aria-label="Sierra de Tolocha con ondas de radio"
    >
      <defs>
        <linearGradient id="sky" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="var(--color-pine-900)" />
          <stop offset="100%" stopColor="var(--color-pine-950)" />
        </linearGradient>
        <linearGradient id="ridge" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="var(--color-pine-600)" />
          <stop offset="100%" stopColor="var(--color-pine-800)" />
        </linearGradient>
        <linearGradient id="mine" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="var(--color-ochre-600)" />
          <stop offset="100%" stopColor="var(--color-ochre-800)" />
        </linearGradient>
        <path id="pine" d="M12 2 L22 22 L15 22 L15 34 L9 34 L9 22 L2 22 Z" />
      </defs>

      <rect width="640" height="360" fill="url(#sky)" />
      <circle cx="520" cy="64" r="30" fill="var(--color-ochre-300)" opacity="0.9" />
      ...
      <path d="M120 290 L260 130 L400 290 Z" fill="var(--color-ochre-400)" opacity="0.6" />
      {/* nieve en la cumbre */}
      <path d="M244 112 L260 80 L276 112 L260 90 Z" fill="var(--color-pine-200)" opacity="0.9" />

      {/* bosque de pinos */}
      <use href="#pine" transform="translate(60, 190) scale(5)" fill="var(--color-pine-500)" />
      <use href="#pine" transform="translate(24, 258) scale(3)" fill="var(--color-pine-600)" />
      <use href="#pine" transform="translate(230, 258) scale(3)" fill="var(--color-pine-600)" />
      <use href="#pine" transform="translate(558, 222) scale(4)" fill="var(--color-pine-500)" opacity="0.85" />
      <use href="#pine" transform="translate(160, 300) scale(1.5)" fill="var(--color-pine-700)" opacity="0.6" />
      <use href="#pine" transform="translate(330, 300) scale(1.6)" fill="var(--color-pine-700)" opacity="0.6" />
      <use href="#pine" transform="translate(450, 312) scale(1.2)" fill="var(--color-pine-700)" opacity="0.5" />
    </svg>
  );
}
    ...
    <div className="flex flex-col gap-12 pb-6 pt-4">
      <section className="flex flex-col items-center gap-10 text-center lg:flex-row lg:text-left">
        <div className="flex-1">
          <h1 className="text-4xl font-bold leading-tight tracking-tight text-pine-100 sm:text-5xl">
            Tolocha<span className="text-ochre-400">Radio</span>
          </h1>
          <p className="mt-3 text-xl font-medium text-pine-200">Exploración radiofónica libre y autoalojada.</p>
          <p className="mt-5 max-w-xl text-pine-300">
          ...
          <div className="mt-7 flex flex-col gap-3 sm:flex-row sm:justify-center lg:justify-start">
            {authenticated ? (
              ...
              <>
                ...
                <Link
                  to="/login"
                  className="inline-flex items-center justify-center gap-2 rounded-xl border border-pine-700 px-6 py-3 text-sm font-medium text-pine-200 transition hover:bg-pine-800"
                >
                  Iniciar sesión
                </Link>
              </>
          ...
          {authenticated ? (
            <p className="mt-3 text-sm text-pine-500">O entra en tu perfil para gestionar tus favoritos.</p>
          ) : (
      ...
      <section className="rounded-2xl border border-pine-800 bg-pine-900/50 p-6">
        <h2 className="text-lg font-semibold text-pine-100">La aplicación permite:</h2>
        <ul className="mt-3 flex flex-col gap-2 text-pine-300">
          {PERMITS.map((permit) => (
      ...
      <section className="max-w-3xl">
        ...
        <p className="mt-5 text-sm uppercase tracking-widest text-pine-400">
          TolochaRadio — radio libre, datos tuyos, control total.
        </p>
      </section>

      <section className="overflow-hidden rounded-2xl border border-pine-800">
        <table className="w-full border-collapse text-sm">
          <caption className="sr-only">Resumen de funciones declaradas</caption>
          <thead>
            <tr className="bg-pine-900/80 text-left text-pine-200">
          ...
          <tbody>
            {FEATURE_ROWS.map((feature) => (
              <tr key={feature} className="border-t border-pine-800">
                <td className="px-4 py-2.5 text-pine-300">{feature}</td>
