...
- [ ] 9.3 GitHub Actions: workflow `ci` (install, typecheck, lint, test, build) en PR/push; workflow `release` que publica en GHCR con tags semVer + `latest` al crear tag `v*`; verificar que el workflow valida en un repo de pruebas (dry-run) y documentar en README cómo despliegue el usuario — *workflows escritos (`.github/workflows/ci.yml` y `release.yml`) y README documentado; pendiente validar en GitHub (requiere cuenta/`gh`)*
- [ ] 9.4 Publicar el proyecto en GitHub: crear `README.md` (qué es TolochaRadio, arquitectura, requisitos, dev local, despliegue con Docker, variables de entorno, uso de la API), licencia y primer commit; verificar que el repo público existe y la documentación es coherente con el despliegue real — *README y LICENSE creados; pendiente `git commit` inicial y publicar (requiere confirmación del usuario y credenciales de GitHub)*
