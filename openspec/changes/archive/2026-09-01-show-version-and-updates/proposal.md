...
- Crear un componente `Footer` que muestre la versión actual debajo de la montaña decorativa.
...
### New Capabilities
- `version-check`: Mostrar versión de la aplicación en el footer y comprobar si hay actualizaciones disponibles en GitHub, mostrando un link al release cuando corresponda.

### Modified Capabilities

## Impact

- **Archivos nuevos**: `apps/web/src/components/Footer.tsx`, `apps/web/src/hooks/useVersionCheck.ts`, `apps/web/src/vite-env.d.ts`.
- **Archivos modificados**: `apps/web/vite.config.ts` (define de versión), `apps/web/src/components/AppShell.tsx` (renderizar Footer).
