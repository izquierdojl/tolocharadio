...
El sistema SHALL aceptar un token de refresco válido y no revocado para emitir un nuevo par de tokens. SHALL rechazar tokens de refresco revocados, expirados o reutilizados tras su rotación.

#### Scenario: Refresco exitoso
- **WHEN** un usuario envía un token de refresco válido al endpoint de renovación
- **THEN** el sistema revoca el token antiguo, emite un nuevo par de tokens y los devuelve
